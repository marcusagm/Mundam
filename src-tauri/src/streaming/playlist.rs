...
pub fn generate_m3u8(file_path: &str, duration_secs: f64, segment_duration: f64) -> String {
...
mod tests {
    ...
    fn test_generate_m3u8_short_video() {
        let m3u8 = generate_m3u8("test.mkv", 25.0, 10.0);

        assert!(m3u8.contains("#EXTM3U"));
        assert!(m3u8.contains("#EXT-X-TARGETDURATION:10"));
        assert!(m3u8.contains("/segment/test.mkv/0.ts"));
        assert!(m3u8.contains("/segment/test.mkv/1.ts"));
        assert!(m3u8.contains("/segment/test.mkv/2.ts"));
        assert!(m3u8.contains("#EXT-X-ENDLIST"));
    }

    #[test]
    fn test_generate_m3u8_exact_duration() {
        let m3u8 = generate_m3u8("video.mkv", 30.0, 10.0);

        // Should have exactly 3 segments for 30 seconds
        assert!(m3u8.contains("/segment/video.mkv/0.ts"));
        assert!(m3u8.contains("/segment/video.mkv/1.ts"));
        assert!(m3u8.contains("/segment/video.mkv/2.ts"));
        assert!(!m3u8.contains("/segment/video.mkv/3.ts"));
    }

    #[test]
    fn test_generate_m3u8_long_video() {
        let m3u8 = generate_m3u8("long.mkv", 3600.0, 10.0);

        // 1 hour = 360 segments
        assert!(m3u8.contains("/segment/long.mkv/359.ts"));
