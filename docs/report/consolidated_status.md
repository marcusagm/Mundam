...
*   **Sincronização do Indexador (`indexer/metadata.rs`):** O indexador ainda utiliza lógica simplificada para detecção de formato. Identificado em `code-review.md`.
    *   *Ação:* Migrar para `crate::formats::FileFormat::detect` para garantir a "Fonte Única da Verdade" (UMDS).
*   **Tratamento de Erros no Worker de Thumbnails (`thumbnail_worker.rs`):** Identificado em `code-review.md`.
...
*   **Renderização de SVGs (`thumbnails/mod.rs`):** Estratégia definida mas não implementada logicamente.
...
### Performance
*   **Otimização de Leitura de Header:** Evitar abrir o arquivo duas vezes (uma para detecção, outra para processamento).
*   **Otimização do Banco de Dados:** Implementar rotina de `VACUUM` e `ANALYZE` para manutenção periódica do SQLite.
...
*   **Preview de Fontes:** Digitação de texto customizado para testar fontes.
...
*   **Taxonomia Hierárquica:** (Backend e Frontend completos).
*   **Visualização Recursiva de Pastas:** (Comandos SQL `WITH RECURSIVE` ativos).
*   **Otimização de Thumbnails:** (Paralelismo Rayon ativo).
*   **Ordenação Global (Global Sorting):** (Implementado no Store e Queries).
*   **Modal de Configurações:** (Refatorado).
*   **Layout Masonry Virtualizado:** (Implementado).
