...
export const DropdownMenu: Component<DropdownMenuProps> = props => {
    ...
    const positionContent = () => {
        ...
        if (top + contentRect.height > viewport.height - 8) {
            top = Math.max(8, triggerRect.top - contentRect.height - 4);
        }
        if (top < 8) top = 8;
    ...
    // Click outside - bridge both trigger and content
    createClickOutside(
...
// Checkbox Item
const MenuCheckboxItem: Component<{
    item: Extract<DropdownMenuItem, { type: 'checkbox' }>;
}> = props => {
    const handleClick = () => {
        if (props.item.disabled) return;
